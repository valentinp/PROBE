...
% drawnow;
        ...
        selectIdx = randperm(size(p_f1_1,2),500);
        ...
        T_wcam_hist_imuonly(:,:,end+1) = T_wcam_hist_imuonly(:,:,end)*inv(T_21_cam);
        % update trajectory
         axes(ha2);
...
translation_imu = NaN(3, size(T_wcam_hist, 3));
for i = 1:size(T_wcam_hist, 3)
    T_wcam =  T_wcam_hist(:, :, i);
    T_wcam_imuonly = T_wcam_hist_imuonly(:,:,i);
    translation(:,i) = T_wcam(1:3, 4);
    translation_imu(:,i) = T_wcam_imuonly(1:3, 4);
end
% 
figure
